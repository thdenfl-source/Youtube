...
**YouTube뿐 아니라 Instagram(릴스) · Facebook · TikTok · X(트위터) 등
`yt-dlp` 가 지원하는 1,000+ 사이트**의 링크를 붙여넣어 받을 수 있습니다.

> ⚠️ **로그인이 필요한 콘텐츠 주의:** 인스타/페북의 **공개** 게시물·릴스는 보통 잘
> 받아지지만, 비공개 계정·연령 제한 등 **로그인이 필요한 영상은 실패**할 수
> 있습니다(사이트 정책). 사이트 변경으로 안 될 땐 `yt-dlp` 를 최신으로
> 업데이트하세요 (`sudo yt-dlp -U` 또는 공식 바이너리 재설치).
...
**`install-mac-autostart.command` 를 한 번만 더블클릭**하면:

- 서버가 **백그라운드에서 항상 실행**되고 (창이 안 떠 있어도 됨)
- 맥을 켜거나 로그인할 때마다 **자동으로 시작**됩니다.

이후로는 터미널을 열 필요 없이 브라우저에서 **http://localhost:3000** 만 열면
바로 사용할 수 있습니다. (북마크 해두면 편리)

끄고 싶을 땐 **`uninstall-mac-autostart.command`** 더블클릭.
